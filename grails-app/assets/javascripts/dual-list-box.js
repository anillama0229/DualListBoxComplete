...
(function($) {
    ...
    function enableDisableUpDown() {
        $(document).on('click', '.selected option', function () {
            if ($('.selected option').length == 1) {
                ...
                if ($('.selected option:selected').val() == $('.selected option:last').val()) {
                    $('.down').prop('disabled', true);
                }else if ($('.selected option:selected').val() == $('.selected option:first').val()) {
                    $('.up').prop('disabled', true);
                }
            }
        });
    ...
    function checkEnableDisableUpDown() {
        if ($('.selected option').length == 1) {
            $('.btn-up-down-sort').prop('disabled', true);
        } else {
            if ($('.selected option:selected').val() == $('.selected option:last').val()) {
    ...
    function createDualListBox(options) {
        ...
        $(options.parentElement).addClass('row').append(
            (options.horizontal == false ? '   <div class="col-md-4">' : '   <div class="col-md-4">') +
            ...
            '       <input class="filter form-control filter-selected" type="hidden" placeholder="Filter" style="margin-bottom: 5px;">' +
    ...
    function createVerticalButtons(copyAllBtn) {
        return '   <div class="col-md-2 center-block" style="margin-top: ' + (copyAllBtn ? '80px' : '130px') +'">' +
            ...
            '       <button type="button" class="btn btn-default col-md-8 col-md-offset-2 stl custom-stl" data-type="stl" style="margin-bottom: 10px;" disabled><span class="glyphicon glyphicon-chevron-left"></span></button>' +
    ...
    function createRightVerticalButtons(copyAllBtn) {
        return '   <div class="col-md-2 center-block" style="margin-top: ' + (copyAllBtn ? '80px' : '130px') +'">' +
    ...
    function handleMovement(options) {
        var highlightSelected = options.hasOwnProperty("highlightSelected") ? options.highlightSelected : false;
        $(options.parentElement + ' .unselected').find('option:selected').prop('selected', false);
        $(options.parentElement + ' .selected').find('option:selected').prop('selected', highlightSelected);
